...
To make it run unattended, with no human clicking *approve*, we started the agent with
...
What we shipped, in four steps. The tool takes one untrusted string, the image
`concept`, from whoever calls it. It puts that string into a prompt. It hands the
