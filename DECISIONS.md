...
## D71 — `ASK` is satisfied per host: native on Claude Code, out-of-band `ApprovalStore` on MCP-only hosts, `DENY` when unattended

> **Renumbered from D41 on 2026-08-15.** This entry was written 2026-07-19 on the
> `approval-capable-hosts` branch and sat unmerged for four weeks, during which `main`
> allocated **D41 to a different decision** (*approval tokens are correlation ids, not
> grants*, 2026-07-23). Both are real and unrelated; this one takes the next free number.
> **A decision number reserved on a branch is not reserved.**
