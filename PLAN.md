...
- [ ] **E14.2** Build pipeline: `wasm-pack`/`wasm-bindgen` target producing a versioned `.wasm` + JS glue, size-checked (target < ~300 KB gz) and reproducible offline; emit the artifact where the Astro site can import it. *(Partial: `wasm-pack build --target nodejs --dev` proven end-to-end; the release/`wasm-opt`/size-gate, a `--target web` bundle for Astro, and the emit step remain.)*
- [ ] **E14.3** Bridge smoke-test island: a bare manifest-editor + event-picker that round-trips through the wasm bundle and prints the raw decision (ALLOW/ASK/DENY/ABSENT/REPLAN + deciding rule) — the thinnest proof the engine works end-to-end in the browser. The *polished, public* visualizations build on this in **E15**. *(Headless equivalent shipped: `crates/harness-wasm/smoke-test.cjs` runs the wasm from Node and asserts the taint floor; the in-browser island is still to do.)*
