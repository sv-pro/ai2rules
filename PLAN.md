...
- [x] **E18.5** ai2rules adapter — `harness project` (D72) + `harness gate` (D24) +
  ...
  with a step-for-step parity check.
...
- [x] **E18.8** Two-directional acceptance — `--assert-contrast` fails both when ai2rules
  fails a scenario and when the reference gateway *passes* one, because a baseline that stops
  failing has stopped measuring anything.
