...
├── DECISIONS.md              # ADR-lite decision log (D1–D68+)
...
harness-wasm 0 · **total 302** (plus the harness-wasm Node smoke tests, run via
wasm-pack)
...
- **Record architectural decisions in `DECISIONS.md`.** When a choice closes off
  ...
  can be revisited later. Currently D1–D68.
...
| `DECISIONS.md` | ADR-lite log D1–D68+; consult before choosing alternatives |
